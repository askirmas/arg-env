...
docker run --rm --env-file=../docker.env docker-env_file > "../../result/docker-env_file.env"

docker build --tag docker-env --file Dockerfile.injected . && \
docker run --rm docker-env > "../../result/docker-env.env"
...
docker-compose run --rm environment > "../../result/compose-environment.env"

## TODO replace with raw env
docker-compose build env_file-shell && \
docker-compose run --rm env_file-shell > "../../result/compose-env_file-shell.env"

docker-compose build env_file-docker && \
docker-compose run --rm env_file-docker > "../../result/compose-env_file-docker.env"
...
# JS stuff
cd ..
./js/dotenv.js output/shell.env > "result/dotenv.env"
./js/dotenv-expand.js docker-compose/.env output/shell.env > "result/dotenv_expanded.env"
